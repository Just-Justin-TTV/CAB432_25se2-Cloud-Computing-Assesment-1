...
sys.path.append("/code")  # <-- makes app1 importable
...
ITERATIONS_DELAY = 2  # delay between checks (seconds)
...
# ----- Start session and authenticate with CSRF -----
session = requests.Session()
login_page = session.get(f"{BASE_URL}/login/")  # fetch CSRF token
...
login_resp = session.post(
    ...
    headers={"Referer": f"{BASE_URL}/login/"}  # important for Django CSRF
)
...
with open(CPU_LOG_FILE, "a") as f:
    while not stop_flag:
        ...
        # Hit a dummy endpoint (keeps request logic if needed)
        try:
            resp = session.get(f"{BASE_URL}/dashboard/")  # example endpoint
        ...
        # Realistic CPU usage across all cores (0-100%)
